...
export default function StyledGoogleCalendar({ events = [], onDateSelect }) {
  ...
  const fcEvents = useMemo(
    () =>
      events
        .filter((e) => e.date) // keep only events that have a date
        .map((e, index) => ({
          id: e.id ?? String(index),
          title: e.title ?? "",
          start: toISOFromMMDDYYYY(e.date), // convert from MM/DD/YYYY
        }))
        .filter((e) => !!e.start), // drop any that failed conversion
    [events]
  );
